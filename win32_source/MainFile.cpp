...
void UpdateSettingsTab(HWND dlgHwnd, int tabID)
{
	...
	SendMessage(ctlA, CB_SETCURSEL, (toSetInID+2), 0); //Choose the first item, for now.
	SendMessage(ctlB, CB_SETCURSEL, (toSetOutID+2), 0); //Choose the first item, for now.
}
...
BOOL CALLBACK SettingsDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		case WM_INITDIALOG:
		{
			...
			SendMessage(ctl, CB_SETCURSEL, (toSetID+2), 0); //Choose the first item, for now.
