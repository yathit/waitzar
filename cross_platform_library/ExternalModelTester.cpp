...
int main(int argc, const char* argv[])
{
	...
	model->typeLetter('k', false, L""); //Transition -->'k'
	model->typeLetter('o', false, L""); //Transition (k) --> 'o'
	model->typeLetter('t', false, L""); //Transition (k-->o) --> 't'
	model->typeLetter('e', false, L""); //Transition (k-->o-->t) --> 'e'
	...
	for (unsigned int i=0; i<strlen(hello); i++) {
		//Is it in the model?
		if (!model->typeLetter(hello[i], false, L"")) {
	...
	for (unsigned int i=0; i<strlen(sgp); i++)
		model->typeLetter(sgp[i], false, L"");
